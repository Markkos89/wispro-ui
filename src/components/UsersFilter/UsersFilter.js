  ...
  useEffect(() => {
    const filterUsers = usersList => {
      ...
      return usersList.filter(
        user =>
          user.firstname.toUpperCase().slice(0, idx) === mayusValue ||
          user.lastname.toUpperCase().slice(0, idx) === mayusValue
      )
    }
    ...
    const orderUsers = usersList => {
      ...
      if (orderBy === 'firstname' || orderBy === 'lastname')
        return usersList.sort((a, b) => sortByName(orderBy, a, b))
    ...
    setFiltredUsersList(orderUsers(filterUsers(originalUserList)))
